...
![Line Simplification Feature](https://geoman-static.onrender.com/assets/copy.gif)
...
You can also enable/disable a button in the toolbar to toggle Line Simplification mode. You can enable (disable) it by setting `copyLayerMode` to `true` (`false`) in the `addControls` options.
